import { ArrowLeft, CheckCircle2, Code2, Sparkles, Target } from "lucide-react";
...
export default async function LessonPage({ params }: { params: Promise<{ trackId: string; lessonKey: string }> }) {
  ...
  if (!lesson) notFound();
  const [{ data: contentRow }, { data: state }] = await Promise.all([
  ...
  if (state) {
    ...
    await supabase.from("lesson_states").insert({
      ...
      status: "available",
      last_opened_at: visitedAt,
    });
  }

  return (
    <main className="lesson-page">
      <header className="lesson-page-header"><Link className="back-link" href={`/tracks/${track.id}`}><ArrowLeft size={14} /> {track.title}</Link><div><span className="eyebrow">{lesson.moduleTitle}</span><h1>{lesson.title}</h1><p>{lesson.summary}</p></div><div className="lesson-header-meta"><span><Target size={14} /> Nota mínima 9,0</span><span>{lesson.estimatedMinutes} min</span></div></header>
      {!content ? <div className="empty-state panel lesson-empty"><Sparkles size={25} /><h3>Esta aula ainda é apenas um resumo</h3><p>Gere agora a explicação, exemplo, exercício, arquivos iniciais e critérios específicos desta aula.</p><AsyncActionButton action={generateLesson.bind(null, track.id, lessonKey)} label="Gerar conteúdo desta aula" /></div> : (
        <div className="lesson-layout"><article className="lesson-reading">
        ...
        </article><aside className="lesson-aside"><article className="panel lesson-ai-card"><Sparkles size={21} /><span className="eyebrow">CRITÉRIOS</span><h3>A IA avaliará</h3><ul>{content.exercise.acceptanceCriteria.map((criterion) => <li key={criterion}>{criterion}</li>)}</ul></article><article className="panel lesson-progress-card"><span className="eyebrow">PROGRESSÃO</span><strong>{state?.best_score ?? "—"}</strong><p>Seu melhor resultado ficará salvo aqui.</p></article></aside></div>
      )}
    </main>
  );
}
